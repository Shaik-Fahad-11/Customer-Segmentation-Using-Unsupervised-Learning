...
function updateDashboard() {
    ...
    // FIX: Update Sidebar Year Dynamically
    const sidebarText = document.getElementById('sidebar-status-text');
    if (sidebarText) {
        sidebarText.innerText = `Running analysis on ${year} trends...`;
    }
...
function generateAIInsights(data, containerId, year) {
    ...
    let genderText = genderGap < 20 ? "Gender distribution is balanced." :
        (data.stats.gender_ratio.Female > data.stats.gender_ratio.Male ? "Female shoppers dominate the traffic." : "Male shoppers are the majority.");

    let incomeText = avgInc > 70 ? `High average income ($${avgInc}k). Focus on premium branding.` :
        `Average income is moderate ($${avgInc}k). Focus on volume sales.`;

    const html = `
        <div class="ai-card">
            <i class="fa-solid fa-lightbulb text-yellow-500 mt-1"></i>
            <div><strong>${topSeg}</strong> is the dominant cluster in ${year}.</div>
        </div>
        <div class="ai-card">
            <i class="fa-solid fa-lightbulb text-yellow-500 mt-1"></i>
            <div>${genderText}</div>
...
function generateComparisonInsights(d1, m1, y1, d2, m2, y2) {
    ...
    let visitText = visitDiff > 0 ?
        `<strong>${m1}</strong> had <strong>${visitDiff}% more visitors</strong> than ${m2}.` :
        `<strong>${m2}</strong> outperformed ${m1} in traffic by <strong>${Math.abs(visitDiff)}%</strong>.`;

    let scoreText = scoreDiff > 0 ?
        `${m1} customers spend more aggressively (Score +${scoreDiff}).` :
        `${m2} shows higher spending engagement (Score +${Math.abs(scoreDiff)}).`;

    const html = `
        <div class="ai-card">
            <i class="fa-solid fa-code-compare text-indigo-500 mt-1"></i>
            <div>${visitText}</div>
        </div>
        <div class="ai-card">
            <i class="fa-solid fa-code-compare text-indigo-500 mt-1"></i>
